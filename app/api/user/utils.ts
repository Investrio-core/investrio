  ...
  try {
    ...
    userDebts.forEach(debt => {
      ...
      while (balance > 0) {
        ...
        let monthly_payment = balance < debt.minPayAmount ? balance + monthly_ir_paid : debt.minPayAmount;
        let previousBalance = balance;
        balance -= (monthly_payment - monthly_ir_paid);
        balance = roundToTwo(balance);

        if (balance >= previousBalance) {
          throw new Error("Balance is not decreasing, potential infinite loop detected");
        }
...
export async function extraPaymentGraph(userId: string) {
  try {
    ...
    let transformed: TransformedData = {
      ...
      combinedBalanceWithFirstMonthInterest: combinedBalanceWithFirstMonthInterest + userData[0].totalInterestPaid,
      minPayAmount: userData[0].monthTotalPayment,
      debtFreeMonth: new Date(userData[userData.length - 1].paymentDate).getTime(),
    ...
    dateGroupMap.forEach((cardDataArray, dateString) => {
      ...
      let combinedRemainingBalance = cardDataArray.reduce((acc, card) => acc + card.remainingBalance, 0);
      let combinedMonthlyPayment = cardDataArray.reduce((acc, card) => acc + card.monthlyPayment, 0);
      let combinedMonthlyInterestPaid = cardDataArray.reduce((acc, card) => acc + card.monthlyInterestPaid, 0);
...
export function paymentGraphForStepTwo(records: PayScheduleData[], useExtraPayAmount: boolean = false): CombineGraphData {
  const singleExtraAmount = useExtraPayAmount ? records[0].extraPayAmount : 0;
  const combinedTitle = records.map(r => r.title);
  let combinedInitialBalance = 0, combinedMonthlyInterestRate = 0, combinedMinPayAmount = 0;
  ...
  let initialBalanceWithFirstMonthInterest = monthlyRecords[0].monthlyInterestPaid + combinedInitialBalance;
  let result = {
    title: combinedTitle,
...
function calculateXPayment(debts: any, extraPayment: boolean): PaymentCalculationResult[] {
  const grouped: { [dateKey: string]: { payments: Payment[], totalInterest: number, userId: string } } = {};
  let totalInitialBalance = debts.reduce((acc: any, debt: any) => acc + debt.initialBalance, 0);
  ...
  try {
    for (const debt of debts) {
      ...
      for (const payment of debt.PaymentSchedule) {
        const dateKey = payment.paymentDate;
        if (!grouped[dateKey]) {
          grouped[dateKey] = { payments: [], totalInterest: 0, userId: user_id };
        }
        grouped[dateKey].payments.push({ ...payment, monthlyInterestRate: monthlyInterestRate });
    ...
    let paymentResults = Object.keys(grouped).sort((a, b) => new Date(a).getTime() - new Date(b).getTime())
      .map(dateKey => {
        cumulativeInterestPaid += grouped[dateKey].totalInterest;
        const monthlyPaymentsSum = grouped[dateKey].payments.reduce((sum, payment) => sum + payment.monthlyPayment, 0);
        cumulativePayments += monthlyPaymentsSum;

        const balance = totalInitialBalance + cumulativeInterestPaid - cumulativePayments;
        return {
          paymentDate: dateKey,
          ...
          balance: balance,
          data: grouped[dateKey].payments.map(({ extraPayAmount, paymentDate, ...rest }) => rest)
        };
      });

    return extraPayment ? applyExtraPaymentsToBalance(paymentResults) : paymentResults;
...
function applyExtraPaymentsToBalance(payments: PaymentCalculationResult[]): PaymentCalculationResult[] {
  let remainingExtraPayment = 0;

  for (let i = 0; i < payments.length; i++) {
    let payment = payments[i];
    let availableExtraPayment = payment.extraPayAmount + remainingExtraPayment;

    // Apply extra payments to the first debt
    availableExtraPayment = applyExtraPaymentToFirstDebt(payment, availableExtraPayment);
    availableExtraPayment = applyExtraPaymentToSubsequentDebts(payment, availableExtraPayment);
...
function recalculateTotalPayment(payment: PaymentCalculationResult): void {
  payment.totalPayment = payment.data.reduce((sum, debt) => sum + debt.monthlyPayment, 0);
}
// Apply Extra Payment to First Debt that is not zero of debts
function applyExtraPaymentToFirstDebt(payment: PaymentCalculationResult, availableExtraPayment: number): number {
  ...
  for (let i = 0; i < payment.data.length; i++) {
    ...
    let appliedExtraPayment = Math.min(debtPayment.remainingBalance, availableExtraPayment); // set up for right order of operations bellow
...
function applyExtraPaymentToSubsequentDebts(payment: PaymentCalculationResult, availableExtraPayment: number): number {
  payment.data.forEach((debt, index) => {
    if (index !== 0 && availableExtraPayment > 0) {
      let appliedExtraPayment = Math.min(debt.remainingBalance, availableExtraPayment);
...
function updateOverallBalance(payment: PaymentCalculationResult, availableExtraPayment: number): number {
...
function calculateScheduleBalance(payments: PaymentCalculationResult[], payment: PaymentCalculationResult, index: number): void {
  if (index > 0) {
    payment.balance = payment.data.reduce((sum, debt) => sum + debt.remainingBalance, 0);
  } else {
    payment.balance = payment.totalInitialBalance + payment.totalInterestPaid - payment.totalPayment;
  }
...
function updateDebtPayments(payments: PaymentCalculationResult[], payment: PaymentCalculationResult, index: number): void {
  for (let debtPayment of payment.data) {
    if (index > 0) {
      let previousPayment = payments[index - 1];
      let previousDebtPayment = previousPayment.data.find(d => d.title === debtPayment.title);
      ...
      if (previousDebtPayment) {
        ...
        debtPayment.monthlyInterestPaid = previousDebtPayment.remainingBalance * debtPayment.monthlyInterestRate;

        // Check if debt is about to be paid off
        let potentialRemainingBalance = previousDebtPayment.remainingBalance - (debtPayment.monthlyPayment - debtPayment.monthlyInterestPaid);
        if (potentialRemainingBalance <= 0) {
          // Adjust monthlyPayment for the final payment
          debtPayment.monthlyPayment = previousDebtPayment.remainingBalance + debtPayment.monthlyInterestPaid;
          debtPayment.remainingBalance = 0;
        } else {
...
function updateTotalInterestPaid(payment: PaymentCalculationResult): void {
  payment.totalInterestPaid = payment.data.reduce((sum, dp) => sum + dp.monthlyInterestPaid, 0);
}
...
export async function snowBallPaymentScheduleCalculator(userId: string) {
  try {
    ...
    return new Response('Error calculating snowball payment schedule', { status: 500 });
  } finally {
    await prisma.$disconnect();
}
...
export async function snowBallData(userId: string) {
  try {
    ...
    const filteredPayments = userDebtsPayments.filter(payment => payment.monthTotalPayment !== 0);
...
export async function StepThreeInfo(userId: string) {
  try {
    ...
    const totalInterestPaidSnowball = snowballRecords.reduce((sum, record) => sum + record.totalInterestPaid, 0);

    const lastSnowballRecordWithZeroBalance = await prisma.snowballPaymentSchedule.findFirst({
      where: { userId, remainingBalance: 0 },
      orderBy: { paymentDate: 'asc' },
      select: { paymentDate: true },
    });

    let lastPaymentDateSnowball = lastSnowballRecordWithZeroBalance ? new Date(lastSnowballRecordWithZeroBalance.paymentDate) : null;

    let monthsFaster = 0;
    if (lastPaymentDateFinancial && lastPaymentDateSnowball) {
      monthsFaster = monthDiff(lastPaymentDateSnowball, lastPaymentDateFinancial);
    }
...
export async function StepTwoPayments(userId: string){
  try {
    ...
    if (data === undefined || data === null || data.length === 0){
      return new Response(JSON.stringify({ message: 'No records found for user!' }), { status: 204 });
    }
