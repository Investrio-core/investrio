...
import { allUserFinancialRecords, snowBallPaymentScheduleCalculator } from "../utils";
...
export async function POST(request: Request){
    ...
    if (titles.length !== uniqueTitles.size) {
        return new Response(JSON.stringify({ error: "Duplicate titles found, cannot add records." }), { status: 400 });
    }
    ...
    await prisma.financialRecord.createMany({
        ...
        try {
            ...
            try {
                ...
                return new Response(JSON.stringify({ error: "Failed when trying to save financial records." }), { status: 400});
            } finally {
